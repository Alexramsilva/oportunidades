...
# -*- coding: utf-8 -*-
# -----------------------------------------------------
# MODELO HOOKE TRADING – ENTRY RADAR (ALTA VOLATILIDAD)
# -----------------------------------------------------
...
TICKERS = ["GAPB.MX","SPY", "GRUMAB.MX","FMTY14.MX", "IAU",  "NVDA","WALMEX.MX", "FUNO11.MX", "CHDRAUIB.MX","QUBT", "QTUM", 
...
def analizar_ticker(ticker):
    ...
    # 🔴 SOLO EXTENSIONES BAJISTAS
    if x_actual >= 0:
        return None

    # Tendencia previa bajista (condición de pre-entry)
    tendencia_ok = df["MA5"].iloc[-1] < df["MA10"].iloc[-1]
    if not tendencia_ok:
        return None

    # Margen de oportunidad (qué tan lejos está del entry)
...
# PROCESAMIENTO
# -----------------------------------------------------
st.info("🔄 Analizando activos bajo régimen de alta volatilidad...")
...
if df_screen.empty:
    st.warning("⚠️ No hay activos con extensión bajista suficiente.")
    st.stop()

# -----------------------------------------------------
# RANKING (LOS MÁS ALEJADOS PRIMERO)
# -----------------------------------------------------
...
# -----------------------------------------------------
# ALERTAS POR MARGEN (NO POR CERCANÍA)
# -----------------------------------------------------
def semaforo_vol(x):
    if x >= 0.85:
...
# Alertas reales (las lejanas, no las críticas)
...
if not oportunidades.empty:
    ...
    st.info("ℹ️ No hay activos suficientemente alejados del entry")
...
st.caption("Ranking 1 = activo MÁS alejado del entry por abajo · mayor margen de compra")
